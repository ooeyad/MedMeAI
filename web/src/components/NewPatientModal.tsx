...
interface Props {
  open: boolean;
  onClose: () => void;
  /** If provided, modal switches to edit mode. */
  existing?: any;
}
...
export function NewPatientModal({ open, onClose, existing }: Props) {
  ...
  const save = useMutation({
    mutationFn: async (body: any) => {
      if (editing) return (await api.patch(`/patients/${existing.id}`, body)).data;
    ...
    onError: (e: any) => setError(e.response?.data?.error?.message || "Failed to save patient"),
  });
  ...
  function onSubmit(e: FormEvent) {
    ...
    for (const key of ["allergies", "chronic_diseases", "current_medications"]) {
      payload[key] = form[key]
        ? form[key].split(",").map((s: string) => s.trim()).filter(Boolean)
        : [];
    }
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-lift max-h-[90vh] overflow-auto">
        <div className="sticky top-0 bg-white z-10 flex items-center justify-between px-5 py-4 border-b border-ink-100">
          <div className="flex items-center gap-2 text-ink-800 font-semibold">
            <UserPlus size={16} /> {editing ? "Edit patient" : "New patient"}
          </div>
        ...
        <form onSubmit={onSubmit} className="p-5 space-y-5">
          <Section title="Identity">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="Full name (English) *" value={form.full_name_en} onChange={(v) => setForm({ ...form, full_name_en: v })} required />
              <Field label="Full name (Arabic)" value={form.full_name_ar} onChange={(v) => setForm({ ...form, full_name_ar: v })} placeholder="الاسم بالعربية" />
              <Field label="Phone *" value={form.phone} onChange={(v) => setForm({ ...form, phone: v })} required placeholder="07XXXXXXXX" />
              ...
              <Select label="Gender" value={form.gender} onChange={(v) => setForm({ ...form, gender: v })} options={[{value:"", label:"—"},{value:"male", label:"Male"},{value:"female", label:"Female"},{value:"other", label:"Other"}]} />
          ...
          <Section title="Medical">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <Field label="Blood type" value={form.blood_type} onChange={(v) => setForm({ ...form, blood_type: v })} placeholder="O+" />
              <Field label="Allergies (comma-separated)" value={form.allergies} onChange={(v) => setForm({ ...form, allergies: v })} placeholder="penicillin, peanuts" />
              <Field label="Chronic conditions" value={form.chronic_diseases} onChange={(v) => setForm({ ...form, chronic_diseases: v })} placeholder="hypertension, asthma" />
              <Field label="Current medications" value={form.current_medications} onChange={(v) => setForm({ ...form, current_medications: v })} placeholder="lisinopril 10mg" />
            </div>
          </Section>
          ...
          <Section title="Emergency contact">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <Field label="Phone" value={form.emergency_contact_phone} onChange={(v) => setForm({ ...form, emergency_contact_phone: v })} />
            </div>
          </Section>
          ...
          <div className="flex justify-end gap-2 pt-2 border-t border-ink-100">
            ...
            >
              {save.isPending ? <Loader2 size={14} className="animate-spin" /> : editing ? <Save size={14} /> : <UserPlus size={14} />}
              {save.isPending ? "Saving…" : editing ? "Save changes" : "Create patient"}
            </button>
          </div>
        </form>
      </div>
